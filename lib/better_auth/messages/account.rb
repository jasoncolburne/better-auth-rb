...
module BetterAuth
  module Messages
    ...
    class DeleteAccountRequestAuthentication
    ...
    class DeleteAccountRequestPayload
      attr_accessor :authentication
    ...
    class DeleteAccountRequest < ClientRequest
      def self.new_request(payload, nonce)
        ClientRequest.new_request(payload, nonce)
      end
      ...
      def self.parse(message)
        ...
        access = ClientAccess.new(nonce: payload_data[:access][:nonce])
        auth_data = payload_data[:request][:authentication]
        authentication = DeleteAccountRequestAuthentication.new(
          device: auth_data[:device],
          identity: auth_data[:identity],
          public_key: auth_data[:publicKey],
          rotation_hash: auth_data[:rotationHash]
        )
        request = DeleteAccountRequestPayload.new(authentication: authentication)
    ...
    class DeleteAccountResponsePayload
      def to_h
        {}
      end
    ...
    class DeleteAccountResponse < ServerResponse
      def self.new_response(payload, server_identity, nonce)
        ServerResponse.new_response(payload, server_identity, nonce)
      end
      ...
      def self.parse(message)
        ...
        response = DeleteAccountResponsePayload.new
